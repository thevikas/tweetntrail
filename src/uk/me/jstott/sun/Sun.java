...
import com.substanceofcode.util.*;
...
public class Sun {
   ...
   */
  private static double morningPhenomenon(double julian,
                                          double latitude,
                                          double longitude,
   ...
   * @param dst true if daylight savings time (summer time) should be taken into
   *            account
   * @return time in minutes from zero Z
   */
  public static Time sunriseTime(double julian,
                                   ...
                                   TimeZone timeZone,
                                   boolean dst) {
    double timeMins = morningPhenomenon(julian,
                                        ll.latitude,
                                        -ll.longitude,
                                        SUNRISE_SUNSET_ZENITH_DISTANCE) +
  ...
  /**
   * Calculate the UTC of morning civil twilight for the given day at the given
   * latitude and longitude on Earth
   *
   * @param julian Julian day
   * @param ll latitude and longitude of observer in degrees
  ...
  public static Time morningCivilTwilightTime(double julian,
    ...
    double timeMins = morningPhenomenon(julian,
                                        ll.getLatitude(),
                                        -ll.getLongitude(),
                                        CIVIL_TWILIGHT_ZENITH_DISTANCE) +
                      (timeZone.getRawOffset() / 60000.0);
    if (dst) timeMins += 60.0;
  ...
  public static Time morningNauticalTwilightTime(double julian,
    ...
    double timeMins = morningPhenomenon(julian,
                                        ll.getLatitude(),
                                        -ll.getLongitude(),
                                        NAUTICAL_TWILIGHT_ZENTITH_DISTANCE) +
                      (timeZone.getRawOffset() / 60000.0);
    if (dst) timeMins += 60.0;
  ...
  /**
   * Calculate the UTC of morning astronomical twilight for the given day at the
   ...
   * @return time in minutes from zero Z
   */
  private static double eveningPhenomenon(double julian,
                                          double latitude,
                                          double longitude,
                                          double zenithDistance) {        
    double t = julianDayToJulianCenturies(julian);
  ...
  public static Time sunsetTime(double julian,
                                  ...
                                  TimeZone timeZone,
                                  boolean dst) {
    double timeMins = eveningPhenomenon(julian,
                                        ll.latitude,
                                        -ll.longitude,
                                        SUNRISE_SUNSET_ZENITH_DISTANCE) +
                      (timeZone.getRawOffset() / 60000.0);
  ...
  /**
   * Calculate the UTC of evening civil twilight for the given day at the given
   * latitude and longitude on Earth
   *
   * @param julian Julian day
   * @param ll latitude and longitude of observer in degrees
  ...
  /**
   * Calculate the UTC of evening astronomical twilight for the given day at the
   * given latitude and longitude on Earth
   *
   * @param julian Julian day
   * @param ll latitude and longitude of observer in degrees
  ...
  private static double equationOfTime(double t) {
    ...
    double y = MathUtil.pow((tan(rad(epsilon) / 2)), 2);
    
    double Etime = y * sin(2 * rad(l0)) - 2 * e * sin(rad(m)) + 4 * e * y *
                   sin(rad(m)) * cos(2 * rad(l0)) - 0.5 * y *
                   y * sin(4 * rad(l0)) - 1.25 * e * e * sin(2 * rad(m));
  ...
  /**
   * calculate the hour angle of the sun for a morning phenomenon for the given
   * latitude
   * 
   ...
   */
  private static double hourAngleMorning(double lat,
                                         double solarDec,
                                         double zenithDistance) {
  ...
  /**
   * Calculate the hour angle of the sun for an evening phenomenon for the given
   * latitude
   *
   * @param lat Latitude of the observer in degrees
   * @param solarDec declination of the Sun in degrees
   ...
   */
  private static double hourAngleEvening(double lat,
                                         double solarDec,
                                         double zenithDistance) {
  ...
  private static double meanObliquityOfEcliptic(double t) {
    return 23 + (26 + (21.448 - t * (46.815 + t * (0.00059 - t * (0.001813)))
        / 60)) / 60;
  }
   ...
   * 
   * @param number of Julian centuries since J2000.0
