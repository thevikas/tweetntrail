...
public class InformationCanvas extends BaseCanvas{
    ...
    protected void paint(Graphics g) {
        ...
        if(position!=null) {
            ...
            if(currentTrack!=null) {
                ...
                else
                {
                    ...
                    Time etaTimeO = new Time(c.get(Calendar.HOUR_OF_DAY)
                            ,c.get(Calendar.MINUTE)
                            ,c.get(Calendar.SECOND));
                    
                    etaTime  = etaTimeO.toString();
        ...
        //FIXME - its ignoring DST, should become aware of it from phone info
        boolean TZdst = false;
        
        sunRiseTime = Sun.sunriseTime(julian, position, gmt, TZdst).toString();
        sunSetTime = Sun.sunsetTime(julian, position, gmt, TZdst).toString();
        
        double durationTimeSecs = (long)currentTrack.getFullTrailDurationMilliSeconds()/1000;
        ...
        if(controller.getSunInfoSetting()) {
            drawNextString(g, LocaleManager.getMessage("information_canvas_sun_rise"), sunRiseTime);
            drawNextString(g, LocaleManager.getMessage("information_canvas_sun_set"), sunSetTime);
        }
