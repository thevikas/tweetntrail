...
public class DisplaySettingsForm extends Form implements CommandListener {
    ...
    private ChoiceGroup sunInfoGroup;
    ...
    public void commandAction(Command command, Displayable displayable) {
        if (command == okCommand) {
            ...
            /** Save sun info shown or now */
            boolean sunInfoOn = sunInfoGroup.isSelected(0);
            if (settings.getShowSunInfo() != sunInfoOn) {
                settings.setShowSunInfo(sunInfoOn);
                // Tell the controller if it should keep the backlight always on
                controller.setSunInfoSetting(sunInfoOn);
            }
    ...
    private void addControls() {
        ...
        //TODO - add support for moon times as well
        String[] sunMoonItems = { LocaleManager.getMessage("display_settings_form_riseset_sun") };
        sunInfoGroup = new ChoiceGroup(LocaleManager.getMessage("display_settings_sun_info"),
                ChoiceGroup.MULTIPLE, sunMoonItems, null);
        if (settings.getShowSunInfo()) {
            sunInfoGroup.setSelectedIndex(0, true);
        }
        
        this.append(sunInfoGroup);
    }
}